...
import { Header, IconButton, FormInput, CardItem, FooterTotal } from "../../components/FoodeaComponents";
...
const CheckOut = ({ navigation, route }) => {
    ...
    function renderCards() {
        return (
            <View>
                {selectedCard && dummyData.myCard.map((item, index) => {
                    return (
                        <CardItem
                            key={`CardPayment-${item.id}`}
                            item={item}
                            isSelected={`${selectedCard?.key}-${selectedCard?.id}` == `CardPayment-${item.id}`}
                            disabled={`${selectedCard?.key}-${selectedCard?.id}` == `CardPayment-${item.id}`}
